...
----------------------------------------------------------------------------------------------------
function SinglePlayer:CreateExplosion(shooterIdParam,weaponIdParam,damage,pos,dir,radius,angle,pressure,holesize,effect,effectScale, minRadius, minPhysRadius, physRadius, explosionType, soundRadius, RadialBlurDistance, Brightness, AmmoName, RadialBlurAmount, BrightnessDistance) // CIG - Muhammad. Added blur, brightness and ammo params
	if (not dir) then
		dir=g_Vectors.up;
	end
	...
	-- CIG BEGIN - Muhammad. if the parameters are not passed in then we use default values
	if (not RadialBlurDistance ) then
		RadialBlurDistance = 0.0
	end
	...
	if (not RadialBlurAmount ) then
		RadialBlurAmount = 0.0
	end

	if (not BrightnessDistance ) then 
		BrightnessDistance = 0.0
	end
	-- CIG END

	
	self.game:ServerExplosion(shooterId, weaponId, damage, pos, dir, radius, angle, pressure, holesize, effect, effectScale, explosionType, minRadius, minPhysRadius, physRadius, soundRadius, 0, RadialBlurDistance, Brightness, AmmoName, RadialBlurAmount, BrightnessDistance); -- CIG - Added explosion effects paramaters, 0 is default for friendlyFire
end

...
function BasicActor:HasSilencedWeapon(weaponId)
	...
	if(item) then
		if(item.weapon and item.weapon.GetAccessory and (item.weapon:GetAccessory("Silencer") or item.weapon:GetAccessory("SilencerPistol")))  then
			return true;
		end
	end
	return false
...
function BasicActor:Kill(hit)
	...
	if (self.SharedSoundSignals) then
		...
		elseif(g_gameRules.IsStealthHealthHit == null or not g_gameRules:IsStealthHealthHit(hit.type)) then

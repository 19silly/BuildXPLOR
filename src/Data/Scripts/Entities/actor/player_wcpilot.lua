...
Player = {
	...
	{	
		...
		--
		fileModel = "Objects/Pilot/Pilot.cdf",
		clientFileModel = "Objects/Pilot/Pilot.cdf",
		fpItemHandsModel = "Objects/Weapons/Agent_fp/Agent_fp_Global.chr",
		objFrozenModel= "Objects/Pilot/Pilot.cdf",
...
--		thrusterAISoundRadius = 38,
	...
	modelSetup =
	{
		hideInFPAttachments = {"head","helmet","visor","eyes","torso","arms"},
		hideInCockpitFPAttachments = {"head","helmet","visor","eyes","torso"},
		hideOnLadderFPAttachments = {"head","helmet","visor","eyes"},
	},
...
function Player:UpdateDraw()	
	...
	elseif (self ~= g_localActor or stats.thirdPerson or stats.isOnLadder) then
		...
		if (self == g_localActor and stats.isOnLadder and not stats.thirdPerson) then
			for i,attachment in pairs(self.modelSetup.hideOnLadderFPAttachments) do
		...
		elseif (stats.followCharacterHead==1 or self.grabParams.entityId or (self:IsOnVehicle() and not ghostPit)) then
			...
			for i,attachment in pairs(self.modelSetup.hideInCockpitFPAttachments) do
				self:HideAttachment(0,attachment,true,false);
			end
		elseif ((stats.firstPersonBody or 0)>0 and not ghostPit) then
			self:DrawSlot(0,1);
			--hide attachments for the body first person model
			for i,attachment in pairs(self.modelSetup.hideInFPAttachments) do
				self:HideAttachment(0,attachment,true,false);
		else
			self:DrawSlot(0,0);
		end

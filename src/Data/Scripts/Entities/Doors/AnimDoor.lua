...
function AnimDoor:GetUsableMessage(userId, idx)
	return "@use_door"
end
...
function AnimDoor:OnUsed(user)
	...
	if (newDir == 1) then
		self:Event_Open();
		-- BroadcastEvent(self, "Open");	event now triggered in DoPlayAnimation; this enables us to receive the event when the door is triggered by something other than Used eg. an area test
	else
		self:Event_Close();
		-- BroadcastEvent(self, "Close");	event now triggered in DoPlayAnimation
	end
end
	...
	if (self.bNoAnims ~= 0 or (self.curAnim ~= "" and self.nDirection ~= 0)) then
		if (not self:IsAnimationRunning(0,0)) then
			...
			if (self.nDirection == -1) then
				...
				self:Activate(0);
				self.bNeedUpdate = 0;
				BroadcastEvent(self, "CloseFinish");
			else
				-- fully open
				--System.Log("Opened");
				if AI then
				...
				self:Activate(0);
				self.bNeedUpdate = 0;
				BroadcastEvent(self, "OpenFinish");
			end
	  end
	end
end

-- Magnetic Attachment Volume
-- Proximity trigger that attaches entities with [X] in their name to the volume (and whatever it is attached to)
-- Option to wait for rest before attaching?
-- [GameToken] to confirm before attaching
-- detach

MagneticAttachmentVolume = 
{
	Properties = 
	{
		DimX = 5,
...
function MagneticAttachmentVolume:OnPropertyChange()
	self:OnReset();
end

function MagneticAttachmentVolume:OnSpawn()
	self:OnReset();
end


function MagneticAttachmentVolume:OnReset()
...
function MagneticAttachmentVolume:Event_Enable()
	self:Enable(true);
...
function MagneticAttachmentVolume:Event_Disable()
	self:Enable(false);

--	BroadcastEvent(self, "Disable");
end

function MagneticAttachmentVolume:Event_Enter(entityId)
...
function MagneticAttachmentVolume:Enable(enable)
  self.enabled=enable;
end

MagneticAttachmentVolume.Flowevents =
{
	Inputs = 
	{
		Enable = {MagneticAttachmentVolume.Event_Enable, "bool"},
		Disable = {MagneticAttachmentVolume.Event_Disable, "bool"},
	},
...
function ProximityTrigger:OnReset()
  ...
  -- precalcs some vars used to test triggering entities based in their names. ("selectedEntity" is a totally misleading name for all this...but i wont change that)
  self.bUsesExactSelectedEntity = false;
  self.bUsesWildcardSelectedEntity = false;
...
function ProximityTrigger:GetUsableMessage(userId, idx)

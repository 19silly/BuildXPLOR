...
function ParticleEffect:OnSpawn()
	-- CIG Begin Make sure the flag is cleared when network sync is enabled otherwise it doesn't work
	if self.Properties.bNetworkSync ~= 0 then
		self:SetFlags(ENTITY_FLAG_CLIENT_ONLY, 2);
	else

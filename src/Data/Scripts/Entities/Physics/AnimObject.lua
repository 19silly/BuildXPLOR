...
AnimObject =
{
	...
	ModelFile = "", -- CIG DGarcia @ IllFonic - added this to delta check model so it only gets loaded if it needs to, prevents weird model popping
};
...
function AnimObject:LoadAndPhysicalizeModel()
	...
	-- CIG DGarcia @ IllFonic - added this to delta check model so it only gets loaded if it needs to, prevents weird model popping
		if (self.ModelFile ~= self.Properties.object_Model)then

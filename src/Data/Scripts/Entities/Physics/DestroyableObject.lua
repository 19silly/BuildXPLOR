...
DestroyableObject =
{
	...
	Properties =
	{
		...
		Breakage = -- => BreakToPieces
		{
			...
			bCloneObject = 0,	-- Create a copy of this object instead of a Breakage object 
		},
		...
		Kythera = {
			-- Whether ships avoid this object
			bIs3DObstacle = 0,
		},		
...
------------------------------------------------------------------------------------------------------
function DestroyableObject:OnSpawn()
...
function DestroyableObject:Explode()
	...
	if( CryAction.IsClient() ) then
		self:BreakToPieces(
			...
			Properties.Breakage.bCloneObject
		);
	end
...
function DestroyableObject:GetUsableMessage(userId, idx)
	if (self.Properties.bUsable == 1 and self.bTemporaryUsable == 1) then
...
-- audio functions
----------------------------------------------------------------------------------------
function DestroyableObject:LoadAudioPreloads(eMethod)
...
----------------------------------------------------------------------------------------
function DestroyableObject:UnloadAudioPreloads()
...
----------------------------------------------------------------------------------------
function DestroyableObject:_LookupAudioTriggerIDs()
...
------------------------------------------------------------------------------------------------------
function DestroyableObject:UpdateAudioProxyOffset()
	if (self.audioAnchorEnt ~= nil) then
...
------------------------------------------------------------------------------------------------------
function DestroyableObject:LinksUpdated()
	if (self.audioAnchorEnt == nil) then

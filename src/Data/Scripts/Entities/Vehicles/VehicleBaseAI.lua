...
function VehicleBaseAI:OnVehicleDestroyedAI()
	...
	-- notify spawn point
	if(self.AI.spawnPointID) then
		local spawnPointEnt = System.GetEntity(self.AI.spawnPointID);
		if(spawnPointEnt) then
			spawnPointEnt:OnDeath(self);
		end
	end
	...
	if (Kyt) then
		-- downgrade vehicle's Kythera entity to just be an obstacle
...
--------------------------------------------------------------------------
-- register to assigned AI Territory and AI Wave entities
function VehicleBaseAI:SetupTerritoryAndWave(territory, wave)

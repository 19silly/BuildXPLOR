...
function MakeARDataAttached( entity )
	if (not entity.Properties) then entity.Properties = {} end
	if (not entity.Properties.ARData) then entity.Properties.ARData = {} end

	entity.Properties.ARData.esARType = "Info";
	entity.Properties.ARData.sAREntity = "";
	entity.Properties.ARData.sARMaterial = "";
	entity.Properties.ARData.vectorARAttachPosition = {x=0,y=0,z=0};
	entity.Properties.ARData.fLongRange = 12;
	entity.Properties.ARData.fShortRange = 6;
	entity.Properties.ARData.fMinRange = 1;

	if (entity.Client.OnInit ~= nil) then entity._ARDataPreOnInit = entity.Client.OnInit end

	function entity.Client:OnInit()
		if (self._ARDataPreOnInit ~= nil) then self:_ARDataPreOnInit() end

		CryAction.ActivateExtensionForGameObject(self.id, "ARDataAttachment", true);
	end
	
end

function MakeARLabel( entity )
	if (not entity.Properties) then entity.Properties = {} end
	if (not entity.Properties.ARLabel) then entity.Properties.ARLabel = {} end

	entity.Properties.ARLabel.sARText = "This is text";
	entity.Properties.ARLabel.sARShortDescription = "This is a short description";
	entity.Properties.ARLabel.sARLongDescription = "This is a long description";
	entity.Properties.ARLabel.esARLabelMovementType = "FacingPlayer";
	entity.Properties.ARLabel.vectorARColor = {x=100, y=100, z=100};
end
...
function MakeUsable( entity )
	...
	function entity:GetUsableMessage(srcId, usableId)
...
EntityCommon.PhysicalizeRigid = function( entity,nSlot,Properties,bActive )
	...
	if (Properties.bArticulated == 1) then
		...
		if (Properties.bRigidBody == 1 or Properties.bRigidBodySimple == 1) then -- CIG Andrew Nguyen : added simple body to flag as rigid type
			physType = PE_RIGID;
		else
			physType = PE_STATIC;
		end
	...
	if (Properties.Simulation) then
		...
		SimulationDst["bRigidBodySimple"] = Properties.bRigidBodySimple; -- CIG Andrew Nguyen
...
-------------------------------------------------------------------------------
-- Add ability for an entity to be a Kythera obstacle.
-------------------------------------------------------------------------------

function MakeKythera3DObstacle( entity )
	if (not entity.Properties) then entity.Properties = {} end
	if (not entity.Properties.Kythera) then entity.Properties.Kythera = {} end
	if (not entity.Properties.Kythera.bIs3DObstacle) then
		entity.Properties.Kythera.bIs3DObstacle = 0;
	end
